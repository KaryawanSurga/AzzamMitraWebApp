import "server-only";
import { customerCreateSchema, customerIdSchema, customerListSchema, customerUpdateSchema, saleMutationSchema, calculateSale, jakartaDate, validateTransactionDate } from "@/domain/sales";
...
export class F2Service {
  ...
  async createCustomer(raw: unknown, owner: OwnerProfile | null): Promise<AppResult<CustomerRecord>> { if (!owner) return unauthorizedResult(); const parsed = customerCreateSchema.safeParse(raw); if (!parsed.success) return validation(parsed.error); try { return { ok: true, data: await this.repository.createCustomer(parsed.data, owner) }; } catch (error) { return failure(error); } }
  async updateCustomer(raw: unknown, owner: OwnerProfile | null): Promise<AppResult<CustomerRecord>> { if (!owner) return unauthorizedResult(); const parsed = customerUpdateSchema.safeParse(raw); if (!parsed.success) return validation(parsed.error); try { const data = await this.repository.updateCustomer(parsed.data, owner); return data ? { ok: true, data } : { ok: false, error: { code: "not_found", message: "Pelanggan tidak ditemukan." } }; } catch (error) { return failure(error); } }
  async archiveCustomer(raw: unknown, owner: OwnerProfile | null): Promise<AppResult<CustomerRecord>> { if (!owner) return unauthorizedResult(); const parsed = customerIdSchema.safeParse(raw); if (!parsed.success) return validation(parsed.error); try { const data = await this.repository.setCustomerActive(parsed.data.id, false, owner); return data ? { ok: true, data } : { ok: false, error: { code: "not_found", message: "Pelanggan tidak ditemukan." } }; } catch (error) { return failure(error); } }
  ...
  async createSale(raw: unknown, owner: OwnerProfile | null): Promise<AppResult<SaleRecord>> {
    ...
    try {
      const existing = await this.repository.findSaleByIdempotencyKey(parsed.data.idempotencyKey); if (existing) return { ok: true, data: existing };
      return { ok: true, data: await this.repository.createSaleAtomic(parsed.data, calculated, owner) };
    } catch (error) {
      if (error instanceof RepositoryConflictError) { try { const existing = await this.repository.findSaleByIdempotencyKey(parsed.data.idempotencyKey); if (existing) return { ok: true, data: existing }; } catch { /* return sanitized conflict */ } }
