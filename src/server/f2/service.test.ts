...
import { RepositoryUnavailableError, type CalculatedSale, type CustomerRecord, type F2Repository, type SaleRecord } from "./repository";
...
class FakeRepository implements F2Repository {
  customers: CustomerRecord[] = []; sales = new Map<string, SaleRecord>(); createSaleCalls = 0; failSale = false;
  async createCustomer(input: { name: string; whatsapp?: string; address?: string; notes?: string }): Promise<CustomerRecord> { const record = { id: crypto.randomUUID(), customerNumber: `CUS-${this.customers.length + 1}`, name: input.name, whatsapp: input.whatsapp ?? null, address: input.address ?? null, notes: input.notes ?? null, isActive: true }; this.customers.push(record); return record; }
  async updateCustomer(input: { id: string; name?: string; whatsapp?: string; address?: string; notes?: string }): Promise<CustomerRecord | null> { const record = this.customers.find(({ id }) => id === input.id); if (!record) return null; Object.assign(record, input); return record; }
  async setCustomerActive(id: string, isActive: boolean): Promise<CustomerRecord | null> { const record = this.customers.find((item) => item.id === id); if (!record) return null; record.isActive = isActive; return record; }
...
describe("F2Service", () => {
  ...
  it("membuat, memperbarui, mencari, dan mengarsipkan pelanggan", async () => { const created = await service.createCustomer({ name: "Budi" }, owner); expect(created.ok).toBe(true); if (!created.ok) return; const updated = await service.updateCustomer({ id: created.data.id, notes: "Prioritas" }, owner); expect(updated).toMatchObject({ ok: true, data: { notes: "Prioritas" } }); expect(await service.listCustomers({ query: "Budi" }, owner)).toMatchObject({ ok: true, data: [{ name: "Budi" }] }); expect(await service.archiveCustomer({ id: created.data.id }, owner)).toMatchObject({ ok: true, data: { isActive: false } }); });
