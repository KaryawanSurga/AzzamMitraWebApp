...
}, (t) => [uniqueIndex("expenses_number_unique").on(t.expenseNumber), index("expenses_date_category_idx").on(t.occurredAt, t.category), check("expenses_amount_positive", sql`${t.amountRupiah} > 0`)]).enableRLS();
...
}, (t) => [uniqueIndex("capital_movements_number_unique").on(t.movementNumber), index("capital_movements_date_idx").on(t.occurredAt), check("capital_movements_amount_positive", sql`${t.amountRupiah} > 0`)]).enableRLS();
