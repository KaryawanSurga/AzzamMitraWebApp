...
export const users = pgTable("users", {
  ...
  ...timestamps,
}, (t) => [uniqueIndex("users_email_unique").on(t.email)]).enableRLS();
...
}, (t) => [uniqueIndex("payments_number_unique").on(t.paymentNumber), uniqueIndex("payments_idempotency_unique").on(t.idempotencyKey), index("payments_sale_idx").on(t.saleId), index("payments_paid_at_idx").on(t.paidAt), check("payments_amount_positive", sql`${t.amountRupiah} > 0`)]).enableRLS();
...
export const deliveries = pgTable("deliveries", {
  ...
  crateQuantity: numeric("crate_quantity", { precision: 12, scale: 3 }).notNull(), receivedCrateQuantity: numeric("received_crate_quantity", { precision: 12, scale: 3 }).default("0").notNull(), dispatchedAt: timestamp("dispatched_at", { withTimezone: true }), receivedAt: timestamp("received_at", { withTimezone: true }), notes: text("notes"), idempotencyKey: text("idempotency_key").notNull(), ...timestamps,
}, (t) => [uniqueIndex("deliveries_number_unique").on(t.deliveryNumber), uniqueIndex("deliveries_idempotency_unique").on(t.idempotencyKey), index("deliveries_sale_idx").on(t.saleId), index("deliveries_status_idx").on(t.status), check("deliveries_quantity_valid", sql`${t.crateQuantity} > 0 and ${t.receivedCrateQuantity} >= 0 and ${t.receivedCrateQuantity} <= ${t.crateQuantity}`)]).enableRLS();

export const crateMovements = pgTable("crate_movements", {
  id: uuid("id").defaultRandom().primaryKey(), movementNumber: text("movement_number").notNull(), customerId: uuid("customer_id").notNull().references(() => customers.id, { onDelete: "restrict" }), saleId: uuid("sale_id").references(() => sales.id, { onDelete: "restrict" }), deliveryId: uuid("delivery_id").references(() => deliveries.id, { onDelete: "restrict" }), type: crateMovementType("type").notNull(), quantity: numeric("quantity", { precision: 12, scale: 3 }).notNull(), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull(), reason: text("reason"), idempotencyKey: text("idempotency_key").notNull(), createdAt: timestamps.createdAt,
}, (t) => [uniqueIndex("crate_movements_number_unique").on(t.movementNumber), uniqueIndex("crate_movements_idempotency_unique").on(t.idempotencyKey), index("crate_movements_customer_date_idx").on(t.customerId, t.occurredAt), check("crate_movements_quantity_positive", sql`${t.quantity} > 0`), check("crate_adjustment_reason_required", sql`${t.type} <> 'adjustment' or length(trim(${t.reason})) > 0`)]).enableRLS();

export const expenses = pgTable("expenses", {
  id: uuid("id").defaultRandom().primaryKey(), expenseNumber: text("expense_number").notNull(), idempotencyKey: text("idempotency_key").notNull(), category: expenseCategory("category").notNull(), amountRupiah: bigint("amount_rupiah", { mode: "number" }).notNull(), method: paymentMethod("method"), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull(), notes: text("notes"), evidencePath: text("evidence_path"), ...timestamps,
}, (t) => [uniqueIndex("expenses_number_unique").on(t.expenseNumber), uniqueIndex("expenses_idempotency_unique").on(t.idempotencyKey), index("expenses_date_category_idx").on(t.occurredAt, t.category), check("expenses_amount_positive", sql`${t.amountRupiah} > 0`)]).enableRLS();

export const capitalMovements = pgTable("capital_movements", {
  id: uuid("id").defaultRandom().primaryKey(), movementNumber: text("movement_number").notNull(), idempotencyKey: text("idempotency_key").notNull(), type: capitalMovementType("type").notNull(), amountRupiah: bigint("amount_rupiah", { mode: "number" }).notNull(), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull(), notes: text("notes"), ...timestamps,
}, (t) => [uniqueIndex("capital_movements_number_unique").on(t.movementNumber), uniqueIndex("capital_movements_idempotency_unique").on(t.idempotencyKey), index("capital_movements_date_idx").on(t.occurredAt), check("capital_movements_amount_positive", sql`${t.amountRupiah} > 0`)]).enableRLS();

export const adjustments = pgTable("adjustments", {
  id: uuid("id").defaultRandom().primaryKey(), adjustmentNumber: text("adjustment_number").notNull(), idempotencyKey: text("idempotency_key").notNull(), saleId: uuid("sale_id").notNull().references(() => sales.id, { onDelete: "restrict" }), type: adjustmentType("type").notNull(), amountRupiah: bigint("amount_rupiah", { mode: "number" }).default(0).notNull(), reason: text("reason").notNull(), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull(), createdBy: uuid("created_by").notNull().references(() => users.id, { onDelete: "restrict" }), createdAt: timestamps.createdAt,
}, (t) => [uniqueIndex("adjustments_number_unique").on(t.adjustmentNumber), uniqueIndex("adjustments_idempotency_unique").on(t.idempotencyKey), index("adjustments_sale_idx").on(t.saleId), check("adjustments_amount_nonnegative", sql`${t.amountRupiah} >= 0`), check("adjustments_reason_not_blank", sql`length(trim(${t.reason})) > 0`)]).enableRLS();
