...
}, (t) => [uniqueIndex("deliveries_number_unique").on(t.deliveryNumber), index("deliveries_sale_idx").on(t.saleId), index("deliveries_status_idx").on(t.status), check("deliveries_quantity_valid", sql`${t.crateQuantity} > 0 and ${t.receivedCrateQuantity} >= 0 and ${t.receivedCrateQuantity} <= ${t.crateQuantity}`)]).enableRLS();
...
}, (t) => [uniqueIndex("crate_movements_number_unique").on(t.movementNumber), index("crate_movements_customer_date_idx").on(t.customerId, t.occurredAt), check("crate_movements_quantity_positive", sql`${t.quantity} > 0`), check("crate_adjustment_reason_required", sql`${t.type} <> 'adjustment' or length(trim(${t.reason})) > 0`)]).enableRLS();
